...
@interface GZHGomokuChessModelTemplate ()
@end

@implementation GZHGomokuChessModelTemplate


+(instancetype)shareTemplete
{
    static GZHGomokuChessModelTemplate * templete=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        templete=[[GZHGomokuChessModelTemplate alloc]init];
    });
    return templete;
}
...
-(NSInteger)calculateScoreWithModelValue:(long long)modelValue
{
  ...
  //  printf("First :%d   Second:%d\n",maxFirstValue,maxSecondValue);
    NSInteger score;
    if ([[GZHGomokuGameEngine game].envisionStack depth]%2==0) {
...
-(NSInteger)calculateFirst:(long long)modelValue
{
    ...
    if ([GZHGomokuGameEngine game].playerFirst&&[[GZHGomokuGameEngine game].envisionStack depth]%2==0) {
